...
public class Main {
	...
	public static void main(String[] args) {
		...
		//try {
			 ...
			 * String descFile = Utils.DATA_DIR + "ctrl\\descFile.txt"; String
			 * destPath = Utils.DATA_DIR + "data\\505\\PUMF";
			 * condGenerator.GenerateConditionalsStepByStep(data,descFile,
			 * destPath);
			 */
			 ...
			 * "CENSUS2006DAAROUNDSTOP.csv"); census.prepareDataColumnStorage();
			 */

			// ###############################################################################
			// To speed up the process, we had to synthesize the population by
			// batch and run
			// the SimPSynz software on each batch.
			// This Create the zonal input file for population synthesis (DAUID
			// , Population)
			// ###############################################################################
			 ...
			 * pathOutput = Utils.DATA_DIR + "\\biogeme\\ctrlWithZones.mod";
			 * String pathHypothesis = Utils.DATA_DIR +
			 * "biogeme\\ctrl\\hypoMultiNestWithZones.txt"; //String
			 * pathHypothesis = Utils.DATA_DIR + "biogeme\\ctrl\\noHypo.txt";
			 * BiogemeControlFileGenerator myCtrlGenerator = new
			 * BiogemeControlFileGenerator();
			 ...
			 * 
			 * //###############################################################
			 * ############################# //prepare OD data for modeling
			 * using multithreading, input: travel survey as CSV file // Also
			 * prepare OD data for creating conditional distribution using the
			 * conditionalGenerator.
			 * //###############################################################
			 * #############################
			 * 
			 * //BE CAREFUL !!! By doing multithreading I am assuming that my
			 * different sub sample does not interact with each other.
			 * //However, the time consuming operation I have been trying to
			 * avoid is going through all possible alternatives and finding the
			 * closest ones. //Therefore, it is no honest to separate in
			 * subsample, because by doing so I am assuming that alternatives in
			 * the other samples are not reachable. //Which is false. Therefore,
			 * the non multithreading function should be used. (it is 8 hours
			 * against 1/2 hours).
			 * 
			 * /*odGatineau.initialize(Utils.DATA_DIR + "\\odGatineau.csv"); int
			 * numberOfLogicalProcessors =
			 * Runtime.getRuntime().availableProcessors() -1;
			 * numberOfLogicalProcessors = 1; System.out.println(
			 * "--computation with: " + numberOfLogicalProcessors +
			 * " logical processors");
			 * odGatineau.processDataMultiThreads(numberOfLogicalProcessors, 2,
			 * myCtrlGenerator);
			 */
			...
			// BE CAREFUL : HYPOTHESIS SHOULD NOT BE CHANGED, HOWEVER IT IS
			// IMPORTANT TO EDIT THE CONTROL FILE
			// BEFORE CALIBRATING THE MODEL WITH BIOGEME : THE FIXED PARAMETER
			// SHOULD BE CHOOSEN, DUMMIES SHOULD SPECIFIED
			// AND
			/*
			 * String pathToModel = Utils.DATA_DIR +
			 * "ptSystem\\ctrlWithZones~1.F12"; mySimulator = new
			 * BiogemeSimulator(myCtrlGenerator);
			 * mySimulator.initialize(Utils.DATA_DIR +
			 * "biogeme\\dataZones2.csv");
			 * mySimulator.importBiogemeModel(pathToModel);
			 * mySimulator.importNest(pathToModel);
			 * mySimulator.applyModelOnTravelSurveyPopulation(Utils.DATA_DIR +
			 * "Outputs\\simuleOD2704.csv",1, true);
			 */
			...
			// ###############################################################################
			// String pathData = Utils.DATA_DIR +
			// "SRMSE//matchedSMGlobalRandom.csv";
			// String pathData = Utils.DATA_DIR +
			// "SRMSE//matchedSMLocalRandom.csv";
			// String pathData = Utils.DATA_DIR +
			// "SRMSE//matchedSMWithoutChoiceSet.csv";
			// String pathData = Utils.DATA_DIR +
			// "SRMSE//matchedSMWithChoiceSet.csv";
			// String pathData = Utils.DATA_DIR +
			// "SRMSE//matchedSMWithCHoiceSetWithBetterLivingLocation.csv";
			/*
			 * String pathData = Utils.DATA_DIR +
			 * "SRMSE//matchedSMWithoutChoiceSet4.csv"; String pathRef =
			 * Utils.DATA_DIR + "SRMSE//odStoUsers.csv"; SRMSE srmse = new
			 * SRMSE(); srmse.getDistributions(pathData, pathRef); double temp =
			 * srmse.computeSRMSE(); System.out.println(temp);
			 * 
			 * 
			 * //###############################################################
			 * ################ //Generate a synthetic population and output
			 * statistical analysis of the goodness //of fit for Gatineau
			 * //###############################################################
			 * ################ /*myWorld.Initialize(true, 1);
			 * System.out.println("--initialization completed"); String
			 * pathToSeeds = Utils.DATA_DIR+"data\\CMA505PUMF2006.csv";
			 * //myWorld.CreatePersonPopulationPoolMetroLevel(Utils.DATA_DIR +
			 * "myPersonPool.csv", pathToSeeds);
			 * //myWorld.CreatePersonPopulationPoolLocalLevel(Utils.DATA_DIR +
			 * "myPersonPool.csv", pathToSeeds);
			 * 
			 ...
			 * myWorld.CreatePersonPopulationPoolLocalLevelMultiThreads(Utils.
			 * DATA_DIR + "myPersonPool.csv",
			 * pathToSeeds,numberOfLogicalProcessors);
			 * myWorld.printMetroMarginalFittingAnalysis(UtilsTS.city,
			 * startTime);
			 */

			// ###############################################################################
			// Generate a synthetic population and output statistical analysis
			// of the goodness
			// of fit for Vancouver
			// ###############################################################################
			/*
			 * myWorld.Initialize(true, 1); System.out.println(
			 * "--initialization completed"); String pathToSeeds =
			 * Utils.DATA_DIR+
			 * "data\\ImportanceSamplingConditionals\\Vancouver.txt";
			 * //myWorld.CreatePersonPopulationPoolMetroLevel(Utils.DATA_DIR +
			 * "myPersonPool.csv", pathToSeeds);
			 * //myWorld.CreatePersonPopulationPoolLocalLevel(Utils.DATA_DIR +
			 * "myPersonPool.csv", pathToSeeds);
			 ...
			 * DATA_DIR + "myPersonPool.csv",
			 * pathToSeeds,numberOfLogicalProcessors);
			 * myWorld.printMetroMarginalFittingAnalysis("Vancouver",
			 * startTime);
			 */

			// Generate conditional distributions at metro level for Vancouver
			/*
			 * String data = Utils.DATA_DIR+
			 * "data\\ImportanceSamplingConditionals\\Vancouver.txt"; String
			 * descFile = Utils.DATA_DIR+
			 * "data\\ImportanceSamplingConditionals\\descFile.txt"; String
			 * zonalData =
			 * "D://Recherche//CharlieWorkspace//PopSynz//data//933VancouverDA.csv";
			 * String destPath = "933";
			 * condGenerator.GenerateConditionalsStepByStep(data,descFile,
			 * zonalData,destPath);
			 */
