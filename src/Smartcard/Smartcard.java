...
public class Smartcard extends BiogemeChoice {
	...
	private int getMostSimilarTrip(int curIndex) throws ParseException {
		...
		for (int i = 0; i < myData.get(UtilsSM.date).size(); i++) {
			...
			if (i != curIndex && curMonth == tempMonth && curRoute.equals(tempRoute)
					&& curDirection.equals(tempDirection) && !tempAlighting.equals(UtilsSM.NOT_DONE)) {
		...
		for (int i = 0; i < dailyData.size(); i++) {
			...
			if (!(curSmartcardTrip.alightingInferrenceCase == UtilsSM.DATA_CORRUPTED_STOP_DONT_EXIST)) {
				...
				if (i != dailyData.size() - 1) {
					...
					GTFSStop nextStop = PublicTransitSystem.myStops.get(nextSmartcardTrip.boardingStop.myId);
	...
	// Case of last trip of the day, we try to buckle it with the first trip of
	// the day or the first trip of the next following day
	/*
	 * else if(dailyData.size() != 1 && i == dailyData.size()-1){
	 * HashMap<String, String> HashMap<String, String> nextTapIn =
	 * dailyData.get(i+1); }
	 */
